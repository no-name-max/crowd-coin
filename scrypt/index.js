...
var checkAsyncArguments = function(args, callback_least_needed_pos, message) {
  ...
  if (callback_index === undefined) {
    if (typeof Promise !== "undefined") return undefined; // if promises are available, don't worry about call backs

    var error = new SyntaxError(
      "No callback function present, and Promises are not available"
    );
    throw error;
  }
...
var checkScryptParametersObject = function(params) {
  ...
  if (typeof params !== "object") {
    var error = new TypeError(
      "Scrypt parameters type is incorrect: It must be a JSON object"
    );
  }

  if (!error && !params.hasOwnProperty("N")) {
    var error = new TypeError(
      "Scrypt params object does not have 'N' property present"
    );
  }

  if (!error && params.N !== parseInt(params.N)) {
    var error = new TypeError(
      "Scrypt params object 'N' property is not an integer"
    );
  }

  if (!error && !params.hasOwnProperty("r")) {
    var error = new TypeError(
      "Scrypt params object does not have 'r' property present"
    );
  }

  if (!error && params.r !== parseInt(params.r)) {
    var error = new TypeError(
      "Scrypt params object 'r' property is not an integer"
    );
  }

  if (!error && !params.hasOwnProperty("p")) {
    var error = new TypeError(
      "Scrypt params object does not have 'p' property present"
    );
  }

  if (!error && params.p !== parseInt(params.p)) {
    var error = new TypeError(
      "Scrypt params object 'p' property is not an integer"
    );
  }
...
var processParamsArguments = function(args) {
  ...
  checkNumberOfArguments(
    args,
    "At least one argument is needed - the maxtime",
    1
  );
      ...
      switch (i) {
        ...
        case 2: //max_memfrac
          if (args[2] < 0.0 || args[2] > 1.0) {
            error = new RangeError(
              propertyName + " must be between 0.0 and 1.0 inclusive"
            );
          }
          break;
      }
...
var processKDFArguments = function(args) {
  checkNumberOfArguments(
    args,
    "At least two arguments are needed - the key and the Scrypt paramaters object",
    2
  );
  ...
  if (typeof args[0] === "string")
    ...
    var error = new TypeError(
      "Key type is incorrect: It can only be of type string or Buffer"
    );
...
var processVerifyArguments = function(args) {
  checkNumberOfArguments(
    args,
    "At least two arguments are needed - the KDF and the key",
    2
  );
  ...
  if (typeof args[0] === "string")
    ...
    var error = new TypeError(
      "KDF type is incorrect: It can only be of type string or Buffer"
    );
...
var processHashArguments = function(args) {
  checkNumberOfArguments(
    args,
    "At least four arguments are needed - the key to hash, the scrypt params object, the output length of the hash and the salt",
    4
  );
  ...
  if (typeof args[3] === "string")
    ...
    var error = new TypeError(
      "Salt type is incorrect: It can only be of type string or Buffer"
    );
...
var scrypt = {
  ...
  params: function() {
    var args = arguments,
      callback_index = checkAsyncArguments(
        args,
        1,
        "At least one argument is needed before the callback - the maxtime"
      );
    ...
    if (callback_index === undefined) {
      ...
      return new Promise(function(resolve, reject) {
        ...
        scryptNative.params(args[0], args[1], args[2], Os.totalmem(), function(
          err,
          params
        ) {
          if (err) {
  ...
  kdf: function() {
    var args = arguments,
      callback_index = checkAsyncArguments(
        args,
        2,
        "At least two arguments are needed before the call back function - the key and the Scrypt parameters object"
      ),
      that = this;
    ...
    if (callback_index === undefined) {
      ...
      Crypto.randomBytes(256, function(err, salt) {
        // Normal async with callback
        if (err)
          // Crypto.randomBytes err
          args[2](err);
        // call callback with error
        else scryptNative.kdf(args[0], args[1], salt, args[2]);
      });
    }
  },
  ...
  verifyKdf: function() {
    var args = arguments,
      callback_index = checkAsyncArguments(
        args,
        2,
        "At least two arguments are needed before the callback function - the KDF and the key"
      );
  ...
  hash: function() {
    var args = arguments,
      callback_index = checkAsyncArguments(
        args,
        4,
        "At least four arguments are needed before the callback - the key to hash, the scrypt params object, the output length of the hash and the salt"
      );
    ...
    if (callback_index === undefined) {
      //Promise
      return new Promise(function(resolve, reject) {
        scryptNative.hash(args[0], args[1], args[2], args[3], function(
          err,
          hash
        ) {
          if (err) {
